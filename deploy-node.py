...
def step4_setup_3xui(domain=None):
...
def step5_configure_grpc(config: dict):
    ...
    try:
        ...
        # SNI targets with HTTP/2 support for Reality-gRPC
        sni_configs = [
            {
                "target": "m.vk.com:443",
                "serverNames": ["m.vk.com", "www.m.vk.com"],
                "serviceName": "/vk/api/notify"
            },
            {
                "target": "api.vk.com:443",
                "serverNames": ["api.vk.com", "www.api.vk.com"],
                "serviceName": "/api/v5.131/method"
            },
            {
                "target": "api-maps.yandex.ru:443",
                "serverNames": ["api-maps.yandex.ru", "maps.yandex.ru"],
                "serviceName": "/maps/api/v1"
            },
            {
                "target": "ads.x5.ru:443",
                "serverNames": ["ads.x5.ru", "www.ads.x5.ru"],
                "serviceName": "/api/v1/ads"
            },
            {
                "target": "max.ru:443",
                "serverNames": ["max.ru", "www.max.ru"],
                "serviceName": "/api/v1/messages"
            },
            {
                "target": "sun6-21.userapi.com:443",
                "serverNames": ["sun6-21.userapi.com"],
                "serviceName": "/api/content"
            }
        ]

        # Randomly select an SNI configuration
        sni_config = random.choice(sni_configs)
        print(f"  Selected SNI target: {sni_config['target']}")

        # Build Reality-gRPC streamSettings with conditional externalProxy
        reality_stream_settings = {
            ...
            "realitySettings": {
                "show": False,
                "xver": 0,
                "target": sni_config["target"],
                "serverNames": sni_config["serverNames"],
            ...
            "grpcSettings": {
                "serviceName": sni_config["serviceName"],
                "authority": "",
                "multiMode": False
            }
        }

        # Add externalProxy only if domain is provided
        if domain:
            reality_stream_settings["externalProxy"] = [
                {
                    "forceTls": "same",
                    "dest": domain,
                    "port": 443,
        ...
        inbounds_to_create = [
            ...
            },
            {
                ...
                "streamSettings": reality_stream_settings,
                "sniffing": {
                    "enabled": True,
                    "destOverride": ["http", "tls"]
                }
    ...
    print(f"  3. VLESS-Reality-gRPC (port 8443) - Reality + gRPC")
...
Examples:
  ...
  sudo ./deploy-node.py --proxy --hostname proxy-node --domain example.com,api.example.com --backend 10.20.30.40
    ...
    for i, (name, func, needs_config, param) in enumerate(steps, 1):
        ...
        # Pass config if step needs it, or param if provided
