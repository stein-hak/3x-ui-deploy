...
def step6_configure_nginx(domain=None):
    ...
    print_step(1, 3, "Creating nginx configuration")

    # Generate nginx config based on working example
    nginx_config = f'''# Auto-generated nginx configuration for {domain}
...
'''
    ...
    try:
        ...
        print(f"  ✓ Enabled in sites-enabled")
    except Exception as e:
        print_error(f"Failed to create symlink: {e}")
        return False

    print_step(3, 3, "Next steps")

    print(f"\n{Colors.BOLD}SSL Certificate Required:{Colors.ENDC}")
    print(f"  The nginx config expects SSL certificates. Run:")
    print(f"  sudo certbot --nginx -d {domain}")
    print()
    print(f"  After obtaining certificate:")
